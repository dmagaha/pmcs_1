...
function ($scope, $window, $location, UserService, AuthenticationService, AlertService) {
    ...
    if ($location.path().indexOf('Front') > -1) {
        $scope.page.boards = false;
        $scope.page.settings = false;
        $scope.display.smallText = ' - Front Desk';
    }
    if ($location.path().indexOf('Shop') > -1) {
        $scope.page.boards = false;
        $scope.page.settings = false;
        $scope.display.smallText = ' - Shop Mgr';
    }
    if ($location.path().indexOf('Tech') > -1) {
        $scope.page.boards = false;
        $scope.page.settings = false;
        $scope.display.smallText = ' - Tech View';
    }
    if ($location.path().indexOf('Report') > -1) {
        $scope.page.boards = false;
        $scope.page.settings = false;
        $scope.display.smallText = ' - Report View';
    }
    if ($location.path().indexOf('Parts') > -1) {
        $scope.page.boards = false;
        $scope.page.settings = false;
        $scope.display.smallText = ' - Parts View';
    }
    if ($location.path().indexOf('Writer') > -1) {
        $scope.page.boards = false;
        $scope.page.settings = false;
        $scope.display.smallText = ' - Writer View';
    }
