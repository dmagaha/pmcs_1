taskBoardControllers.controller('RepairOrderFrontDeskCtrl',
...
function ($scope, $interval, RepairOrderService, TeamService, UserService) {
    ...
    $scope.openAddCheckout = function(ro) {
	alert("in add checkout");
        $msg = "Are you sure that " + ro.repair_order_number + " has been checked out? ";
        noty({
            text: $msg,
            layout: 'center',
            type: 'information',
            ...
            buttons: [
                {
                    ...
                    onClick: function($noty) {
                        ...
                        RepairOrderService.checkOut(ro)
                        .success(function(data) {
                            $scope.alerts.showAlerts(data.alerts);
                        });
    ...
    var pendingROResponse = false,
        retryROCount = 3,
        pendingCompletedROResponse = false,
        retryCompletedROCount = 3,
	...
	loadTechs = function() {
        ...
        loadCompletedRepairOrders = function() {
            if (pendingCompletedROResponse) {
                return;
            }

            pendingCompletedROResponse = true;
            RepairOrderService.getCompletedRepairOrders()
            .success(function(data) {
                $scope.updateCompletedRepairOrders(data.data);
                pendingCompletedROResponse = false;
                retryCompletedROCount = 3;
            })
            .error(function() {
                if (retryCompletedROCount--) {
                    pendingCompletedROResponse = false;
                    return;
                }

                $interval.cancel($scope.interval);
                $scope.$parent.loadingCompletedRepairOrders = false;
            });
        },
        loadTeams = function() {
    ...
    $scope.interval = $interval(loadCompletedRepairOrders, 5000);
    ...
    var convertDates = function(timestampArray) {
        ...
        updateRO = function(ro) {
            ...
            $scope.viewRepairOrder.partsOrdered = ro.parts_ordered;
            $scope.viewRepairOrder.backOrdered = ro.back_ordered;
            $scope.viewRepairOrder.criticalBackOrder = ro.critical_back_order;
    ...
    $scope.openPartsOrdered = function(ro, openModal) {
