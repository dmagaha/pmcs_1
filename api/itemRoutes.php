<?php
use RedBeanPHP\R;
// Create new repair order on board
$app->post('/boards/ro', function() use($app, $jsonResponse) {
    $data = json_decode($app->environment['slim.input']);
    $actor1 = getUser();
    logAction($actor1->username . ' called add repair order ', null, null, 1);

    if (validateToken()) {
        logAction($actor1->username . ' token validated ', null, null, 1);
        // add to main board
        //$board = getBoard('Main - Repair');
        $board = R::load('board', 1);
        logAction($actor1->username . ' token validated ', null, $board->id, $board->name);
        // if ($board->id) {
            $ro = R::dispense('ro');
            $ro->repairOrderNumber = $data->repairOrderNumber;
            $ro->year = $data->year;
            $ro->make = $data->make;
            $ro->model = $data->model;
            $ro->color = $data->color;
            ...
            $board->xownLane[0]->ownRos[] = $ro;
            R::store($board);
            // runAutoActions($item);

            if ($ro->id) {
                $actor = getUser();
                logAction($actor->username . ' added item ' . $ro->repairOrderNumber. ' to board ' . $board->name,
                          null, $ro->export(), $ro->id);
        ...
        logAction($actor1->username . ' token not validated ', null, null, 1);
    }
...
$app->post('/items/positions', function() use ($app, $jsonResponse) {
    ...
    if (validateToken()) {
        ...
        if (null != $movedItem) {
            ...
            // check to see if the lane is the repair lane
	    // create a repairItem
	    // link the repairItem with the management item
            // get the repairBoard id
	    // lane would be the first one hardcoded
            // copy the code from the 'api/boards/boardId/items' post
	    // add it to the repair board
        }
        $jsonResponse->addBeans(getBoards());
    }
    $app->response->setBody($jsonResponse->asJson());
