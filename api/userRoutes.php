...
// Get current user
$app->get('/users/tech', function() use($app, $jsonResponse) {
    if (validateToken()) {
        $jsonResponse->addBeans(getTechs());
    }
    $app->response->setBody($jsonResponse->asJson());
});
...
$app->post('/users', function() use($app, $jsonResponse) {
    ...
    if (validateToken(true)) {
        ...
        if (null != $nameTaken) {
            ...
            $user->isWriter = $data->isWriter;
            $user->isTech = $data->isTech;
            $user->isManager = $data->isManager;
            $user->isSuperUser = $data->isSuperUser;
            $user->email = $data->email;
            $user->defaultBoard = $data->defaultBoard;
            $user->team = $data->team;
            $user->currentWorkload = $data->currentWorkload;
            $user->maxWorkload = $data->maxWorkload;
